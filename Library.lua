...
local BaseURL = "https://raw.githubusercontent.com/VoidGrok/VoidKing/main/"
local CustomImageManager = {}
local CustomImageManagerAssets = {
    TransparencyTexture = {
        RobloxId = 139785960036434,
        Path = "VoidKing/assets/TransparencyTexture.png",
    ...
    SaturationMap = {
        RobloxId = 4155801252,
        Path = "VoidKing/assets/SaturationMap.png",
    ...
    LoadingIcon = {
        ...
        Path = "VoidKing/assets/LoadingIcon.png",
    ...
    CheckIcon = {
        ...
        Path = "VoidKing/assets/CheckIcon.png",
...
do
    ...
    function CustomImageManager.AddAsset(
        ...
        CustomImageManagerAssets[AssetName] = {
            ...
            Path = string.format("VoidKing/custom_assets/%s", AssetName),
            URL = URL,
...
local Library = {
    ...
    Scheme = {
        ...
        AccentColor = Color3.fromRGB(0, 70, 140),
...
local Templates = {
    ...
    Window = {
        ...
        --// Floating Button (abre/fecha) \\--
        FloatShape = "Squircle", -- "Circle" | "Squircle" | "Square"
        FloatStrokeColor = Color3.fromRGB(12, 22, 45), -- #0c162d
        FloatBackgroundColor = Color3.fromRGB(14, 10, 22),
        FloatBackgroundTransparency = 0,
        DefaultIcon = "shield", -- usado se Icon estiver vazio/nil
    },
    Dialog = {
        Title = "Dialogo",
...
function Library:MakeOutline(Frame: GuiObject, Corner: number?, ZIndex: number?)
    warn("VoidKing:MakeOutline esta obsoleto, use VoidKing:AddOutline.")
    ...
    function Funcs:AddButton(...)
        ...
        local function CreateButton(Button)
            ...
            table.insert(
                Library.Corners,
                New("UICorner", {
                    CornerRadius = UDim.new(0, math.max(Library.CornerRadius, 6)),
                    Parent = Base,
                })
            )
...
function Library:CreateWindow(WindowInfo)
    ...
    -- Botao flutuante com logo (abre/fecha + arrastavel)
    -- Tamanho mantido: 54x54 (icone ~50x50)
    do
        local FloatImage = ""
        local FloatRectOffset = Vector2.zero
        local FloatRectSize = Vector2.zero
        local IsCustomAsset = false

        local function applyIcon(parsed)
            if parsed and parsed.Url then
                FloatImage = parsed.Url
                FloatRectOffset = parsed.ImageRectOffset or Vector2.zero
                FloatRectSize = parsed.ImageRectSize or Vector2.zero
                IsCustomAsset = parsed.Custom == true
                return true
            end
            return false
        end

        -- 1) Tenta o Icon passado no CreateWindow
        -- 2) Se vazio/nil → usa DefaultIcon da Library
        local used = false
        local iconSource = WindowInfo.Icon
        if iconSource == nil or iconSource == "" or iconSource == false then
            iconSource = WindowInfo.DefaultIcon or "shield"
        end
        if iconSource then
            used = applyIcon(Library:GetCustomIcon(iconSource))
        end
        if not used then
            used = applyIcon(Library:GetIcon("shield")) or applyIcon(Library:GetCustomIcon("shield"))
        end

        -- Forma do botao
        local shape = string.lower(tostring(WindowInfo.FloatShape or "squircle"))
        local cornerRadius
        if shape == "circle" then
            cornerRadius = UDim.new(1, 0)
        elseif shape == "square" then
            cornerRadius = UDim.new(0, 4)
        else -- squircle (padrao)
            cornerRadius = UDim.new(0, 14)
        end

        local bgColor = WindowInfo.FloatBackgroundColor or Color3.fromRGB(14, 10, 22)
        local bgTransparency = WindowInfo.FloatBackgroundTransparency
        if bgTransparency == nil then
            -- Se for imagem custom (rbxassetid), deixa o fundo mais transparente
            -- para reduzir o preto visivel nas beiradas
            bgTransparency = IsCustomAsset and 0.35 or 0
        end
        local strokeColor = WindowInfo.FloatStrokeColor or Color3.fromRGB(12, 22, 45) -- #0c162d
        ...
        -- Imagem quase preenchendo o botao (50x50 dentro de 54x54)
        -- + mesmo corner radius → corta o preto das laterais
        local FloatIcon = New("ImageLabel", {
            ...
            ImageRectOffset = FloatRectOffset,
            ImageRectSize = FloatRectSize,
            ScaleType = Enum.ScaleType.Crop, -- Crop cobre melhor que Fit
            ZIndex = 10001,
            Parent = FloatBtn,
        })
        local FloatIconCorner = New("UICorner", {
            CornerRadius = cornerRadius,
            Parent = FloatIcon,
        })

        -- Guardar referencias para poder mudar depois (Configs / API)
        Library.FloatButton = FloatBtn
        Library.FloatIcon = FloatIcon
        Library.FloatStroke = FloatStroke
        Library.FloatCorner = FloatCorner
        Library.FloatIconCorner = FloatIconCorner
        Library.FloatShape = shape

        -- API publica para mudar forma/cor/icone em runtime
        function Library:SetFloatShape(newShape)
            newShape = string.lower(tostring(newShape or "squircle"))
            local r
            if newShape == "circle" then
                r = UDim.new(1, 0)
            ...
            Library.FloatShape = newShape
            if Library.FloatCorner then Library.FloatCorner.CornerRadius = r end
            if Library.FloatIconCorner then Library.FloatIconCorner.CornerRadius = r end
        end

        function Library:SetFloatStrokeColor(color)
            if typeof(color) == "Color3" and Library.FloatStroke then
                Library.FloatStroke.Color = color
            end
        end

        function Library:SetFloatBackgroundColor(color, transparency)
            if Library.FloatButton then
                if typeof(color) == "Color3" then
                    Library.FloatButton.BackgroundColor3 = color
                end
                if typeof(transparency) == "number" then
                    Library.FloatButton.BackgroundTransparency = transparency
                end
            end
        end
        ...
        function Library:SetFloatIcon(icon)
            ...
            if not parsed or not parsed.Url then
                parsed = Library:GetIcon(tostring(icon or "shield"))
            end
            if parsed and parsed.Url and Library.FloatIcon then
                Library.FloatIcon.Image = parsed.Url
                Library.FloatIcon.ImageRectOffset = parsed.ImageRectOffset or Vector2.zero
                Library.FloatIcon.ImageRectSize = parsed.ImageRectSize or Vector2.zero
            end
        end
        ...
        Library:GiveSignal(UserInputService.InputChanged:Connect(function(input)
            if floatDrag and (input.UserInputType == Enum.UserInputType.MouseMovement
                or input.UserInputType == Enum.UserInputType.Touch) then
        ...
        Library:GiveSignal(UserInputService.InputEnded:Connect(function(input)
            if input.UserInputType == Enum.UserInputType.MouseButton1
                or input.UserInputType == Enum.UserInputType.Touch then
                if floatDrag and not wasDragged and clickStart and (input.Position - clickStart).Magnitude < 8 then
                    Library:Toggle()
                end
                floatDrag = false
            end
...
function Library:CreateLoading(LoadingInfo)
    if Library.ActiveLoading then
        warn("Janela de carregamento ja existe, nao e possivel criar multiplas.")
