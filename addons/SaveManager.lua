...
local function GetFolderPath(): false | string
    ...
    return string.format("%s/Configs", SaveManager.Folder)
end
...
local function GetSubFolderPath(): false | string
    ...
    return string.format("%s/Configs/%s", SaveManager.Folder, SaveManager.SubFolder)
end
...
function SaveManager:SetSubFolder(SubFolder: string)
    assert(IsValidFolderPath(SubFolder), "Caminho invalido")
...
function SaveManager:RefreshConfigList()
    ...
    if not (SuccessList and typeof(Files) == "table") then
        SaveManager.Library:Notify(string.format("Falha ao carregar lista de configs: %s", tostring(Files)))
        return {}
    end
...
function SaveManager:Save(ConfigName: string): (boolean, string?)
    if IsStringEmpty(ConfigName) then
        return false, "Nome de config invalido"
    end
    ...
    if not SuccessWrite then
        return false, "Falha ao escrever arquivo de config: " .. tostring(ErrorMessage)
    end
...
function SaveManager:Load(ConfigName: string): (boolean, string?)
    if IsStringEmpty(ConfigName) then
        return false, "Nenhuma config selecionada"
    end

    local ConfigPath = GetConfigPath(ConfigName)
    if ConfigPath == false or not isfile(ConfigPath) then
        return false, "Arquivo de config nao existe"
    end

    local SuccessRead, Content = pcall(readfile, ConfigPath)
    if not SuccessRead then
        return false, "Falha ao ler arquivo de config"
    end
    ...
    if not SuccessDecode or typeof(Decoded) ~= "table" or typeof(Decoded.objects) ~= "table" then
        return false, "Falha ao decodificar dados da config"
    end
...
function SaveManager:Deletar(ConfigName: string): (boolean | string?)
    if IsStringEmpty(ConfigName) then
        return false, "Nenhuma config selecionada"
    end
    ...
    local SuccessDeletar, ErrorMessage = pcall(delfile, ConfigPath)
    if not SuccessDeletar then
        return false, "Falha ao deletar arquivo de config: " .. tostring(ErrorMessage)
    end

    if ConfigName == SaveManager.AutoloadConfig then
        SaveManager:DeletarAutoLoadConfig()
    end
...
function SaveManager:GetAutoloadConfig(): (string, boolean, string?)
    ...
    if not isfile(AutoloadPath) then
        return "none", false, "Config de autoload nao definida"
    end
    ...
    if not ConfigExists then
        return "none", false, "Arquivo de config nao encontrado"
    end
...
function SaveManager:SaveAutoloadConfig(ConfigName: string): (boolean, string?)
    ...
    if not DoesConfigExist(ConfigName) then
        return false, "Config nao existe"
    end
...
function SaveManager:LoadAutoloadConfig()
    ...
    if not Success or FetchErrorMessage then
        if FetchErrorMessage ~= "Config de autoload nao definida" then
            SaveManager.Library:Notify(string.format("Falha ao carregar autoload: %s", FetchErrorMessage))
        end
    ...
    if not SuccessLoad then
        SaveManager.Library:Notify(string.format("Falha ao carregar autoload: %s", LoadErrorMessage))
        return
    end

    SaveManager.Library:Notify(string.format("Autoload %q carregado com sucesso", ConfigName))
end

function SaveManager:DeletarAutoLoadConfig(): (boolean, string?)
    SaveManager:CheckFolderTree()
    ...
    if not isfile(AutoloadPath) then
        return false, "Config de autoload nao definida"
    end

    local SuccessDeletar, ErrorMessage = pcall(delfile, AutoloadPath)
    if not SuccessDeletar then
        return false, ErrorMessage
    end
...
function SaveManager:BuildConfigSection(Tab: any, IconName: string)
    ...
    local ConfiguracaoBox = Tab:AddRightGroupbox("Configuracao", IconName or "folder-cog")
    ...
    local function RefreshAutoloadConfigLabel()
        ...
        AutoloadConfigLabel:SetText(string.format("Config de autoload atual: %s", AutoloadConfigName))
        if ConfigList then RefreshList() end
    end

    --// Create
    ConfiguracaoBox:AddInput("SaveManager_ConfigName", {
        Text = "Nome da config"
    })

    ConfiguracaoBox:AddButton("Criar config", function()
        local ConfigName = ConfigNameInput.Value
        if IsStringEmpty(ConfigName) then
            SaveManager.Library:Notify("Nome da configuracao nao pode ser vazio.")
            return
        end

        if string.lower(ConfigName) == "autoload" then
            SaveManager.Library:Notify("Nome de config invalido.")
            return
        end
        ...
        ShowDialog(
            ...
            "Config ja existe",
            string.format("Uma config chamada %q ja existe. Sobrescrever vai substituir pelas configuracoes atuais.", ConfigName),

            "Sobrescrever",
            function()
                local Success, ErrorMessage = SaveManager:Save(ConfigName)
                if not Success then
                    SaveManager.Library:Notify(string.format("Falha ao criar config %q: %s", ConfigName, ErrorMessage))
                    return
                end

                SaveManager.Library:Notify(string.format("Config %q criada com sucesso", ConfigName))
                RefreshList()
            end
        )
    end)

    ConfiguracaoBox:AddDivider()

    --// Manage
    ConfiguracaoBox:AddDropdown("SaveManager_ConfigList", {
        Text = "Lista de configs",
    ...
    ConfiguracaoBox:AddButton({
        ...
        Func = function()
            ...
            if IsStringEmpty(ConfigName) then
                SaveManager.Library:Notify("Selecione uma config primeiro.")
                return
            end

            local Success, ErrorMessage = SaveManager:Load(ConfigName)
            if not Success then
                SaveManager.Library:Notify(string.format("Falha ao carregar config %q: %s", ConfigName, ErrorMessage))
                return
            end

            SaveManager.Library:Notify(string.format("Config %q carregada com sucesso", ConfigName))
        end
    })
    
    ConfiguracaoBox:AddButton({
        Text = "Sobrescrever config",
        DoubleClick = false,
        ...
        Func = function()
            ...
            ShowDialog(
                ...
                "SaveManager_SobrescreverConfig",
                "Sobrescrever config",
                string.format("Tem certeza que deseja sobrescrever %q com as configuracoes atuais? Nao pode ser desfeito.", ConfigName),

                "Sobrescrever",
                function()
                    local Success, ErrorMessage = SaveManager:Save(ConfigName)
                    if not Success then
                        SaveManager.Library:Notify(string.format("Falha ao sobrescrever config %q: %s", ConfigName, ErrorMessage))
                        return
                    end

                    SaveManager.Library:Notify(string.format("Config %q sobrescrita com sucesso", ConfigName))
                end
            )
        end
    })
    ...
    ConfiguracaoBox:AddButton({
        ...
        Func = function()
            ...
            ShowDialog(
                ...
                "SaveManager_DeletarConfig",
                "Deletar config",
                string.format("Tem certeza que deseja deletar %q? Nao pode ser desfeito.", ConfigName),
                
                "Deletar",
                function()
                    local Success, ErrorMessage = SaveManager:Deletar(ConfigName)
                    if not Success then
                        SaveManager.Library:Notify(string.format("Falha ao deletar config %q: %s", ConfigName, ErrorMessage))
                        return
                    end

                    SaveManager.Library:Notify(string.format("Config %q deletada com sucesso", ConfigName))
    ...
    ConfiguracaoBox:AddButton("Atualizar lista", RefreshList)

    --// Autoload Config
    ConfiguracaoBox:AddButton({
        Text = "Definir como autoload",
        DoubleClick = false,
        ...
        Func = function()
            ...
            if not Success then
                SaveManager.Library:Notify(string.format("Falha ao definir autoload %q: %s", ConfigName, ErrorMessage))
                return
            end

            SaveManager.Library:Notify(string.format("Config de autoload definida para %q", ConfigName))
            RefreshAutoloadConfigLabel()
        end
    })

    ConfiguracaoBox:AddButton({
        Text = "Resetarar autoload",
        DoubleClick = false,
        ...
        Func = function()
            ShowDialog(
                ...
                "SaveManager_ResetarAutoload",
                "Resetarar autoload config",
                "Tem certeza que deseja limpar a config de autoload? Nenhuma config sera carregada automaticamente na proxima vez.",
                
                "Resetar",
                function()
                    local Success, ErrorMessage = SaveManager:DeletarAutoLoadConfig()
                    if not Success then
                        SaveManager.Library:Notify(string.format("Falha ao resetar autoload: %s", ErrorMessage))
                        return
                    end

                    SaveManager.Library:Notify("Autoload resetado com sucesso.")
                    RefreshAutoloadConfigLabel()
                end
            )
        end
    ...
    AutoloadConfigLabel = ConfiguracaoBox:AddLabel("Config de autoload atual: ...", true);
