...
function KeySystem:Setup(Window, Config)
    ...
    KeyTab:AddKeyBox(function(ReceivedKey)
        if not ReceivedKey or ReceivedKey == "" then
            ...
            Library:Notify({
                Title = "Key System",
                Description = "Digite uma key valida.",
                Time = 3,
            })
            return
        end
        ...
        Library:Notify({
            Title = "Key System",
            Description = "Verificando key...",
            Time = 2,
        })

        task.spawn(function()
            local ok = redeemKey(ReceivedKey)
            if ok then
                KeySystem.Validated = true
                if Config.Secret then
                    _G[Config.Secret] = true
                end
                statusLabel:SetText("Status: key valida!")
                Library:Notify({
                    Title = "Sucesso",
                    Description = "Key valida! Script liberado.",
                    Time = 4,
                })
                ...
                statusLabel:SetText("Status: key invalida")
                Library:Notify({
                    Title = "Erro",
                    Description = "Key invalida ou expirada.",
                    Time = 4,
                })
            end
        end)
        ...
        Func = function()
            task.spawn(function()
                ...
                if ok and link then
                    ...
                    Library:Notify({
                        Title = "GET KEY",
                        Description = tostring(link) or "Erro ao gerar link.",
                        Time = 3,
                    })
                end
            end)
    ...
    if Config.DiscordURL then
        KeyTab:AddButton({
            Text = "DISCORD (copiar convite)",
            Func = function()
                fSetClipboard(Config.DiscordURL)
                Library:Notify({
                    Title = "Discord",
                    Description = "Convite copiado!",
                    Time = 3,
                })
            end,
        })
    ...
    task.spawn(function()
        ...
        if saved then
            ...
            local ok = redeemKey(saved)
            if ok then
                KeySystem.Validated = true
                if Config.Secret then
                    _G[Config.Secret] = true
                end
                statusLabel:SetText("Status: key valida (auto-login)!")
                Library:Notify({
                    Title = "Auto-login",
                    Description = "Key salva validada com sucesso.",
                    Time = 3,
                })
                if typeof(Config.OnSuccess) == "function" then
                    task.defer(Config.OnSuccess)
                ...
                statusLabel:SetText("Status: key salva invalida")
            end
        end
    end)
